        ...
        if (yMarioD > -this.Height && yMarioD < Mario.MarioCharacter.Height) {
            if (this.Type !== Mario.Enemy.Spiky && Mario.MarioCharacter.Ya > 0 && yMarioD <= 0 && (!Mario.MarioCharacter.OnGround || !Mario.MarioCharacter.WasOnGround)) {
                ...
                if (this.Winged) {
                    this.Winged = false;
                    this.Ya = 0;
                    GlobalInfo.data.log({a: 'enemy_dewinged', t: this.Type}, true);
                } else {
                    this.YPicO = 31 - (32 - 8);
                    this.PicHeight = 8;
                    ...
                    GlobalInfo.data.log({a: 'enemy_jump_killed', t: this.Type}, true);
                }
            } else {
                Mario.MarioCharacter.GetHurt(this.Type);
            }
        }
    }
};
...
Mario.Enemy.prototype.ShellCollideCheck = function(shell) {
    ...
    if (xd > -16 && xd < 16) {
        if (yd > -this.Height && yd < shell.Height) {
            ...
            this.Winged = false;
            this.YFlip = true;
            GlobalInfo.data.log({a: 'enemy_shell_killed', t: this.Type}, true);
            return true;
        }
    }
    return false;
...
Mario.Enemy.prototype.FireballCollideCheck = function(fireball) {
    ...
    if (xd > -16 && xd < 16) {
        if (yd > -this.Height && yd < fireball.Height) {
            ...
            this.Winged = false;
            this.YFlip = true;
            GlobalInfo.data.log({a: 'enemy_fireball_killed', t: this.Type}, true);
            return true;
        }
    }
};
...
Mario.Enemy.prototype.BumpCheck = function(xTile, yTile) {
    ...
    if (this.X + this.Width > xTile * 16 && this.X - this.Width < xTile * 16 + 16 && yTile === ((this.Y - 1) / 16) | 0) {
        ...
        this.Winged = false;
        this.YFlip = true;
        GlobalInfo.data.log({a: 'enemy_bump_killed', t: this.Type}, true);
    }
};
